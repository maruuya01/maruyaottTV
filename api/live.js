export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      return res.status(500).send("Invalid JSON format in ch.js");
    }
    ...
    for (const ch of channels) {
      if (!ch || typeof ch !== 'object' || !ch.title || !ch.file) continue;
    ...
    res.status(500).send("Failed to load or parse channel list");
  }
}
